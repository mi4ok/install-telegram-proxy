...
class TgBot {
     ...
     */
    getToken() {
        const botConnection = new SimpleRecord('itsm_telegram_bot_connection');
    ...
    __setDefaultPOSTRequest(url, body) {
        ...
        if (status !== 200) {
            ss.error(`[TgBot] POST ${url} failed: status=${status}, body=${responseBody}`);
        } else {
            ss.info(`[TgBot] POST ${url} ok: ${responseBody}`);
        }
    }
     ...
     */
    __getApiUrl() {
        const botUrl = ss.getProperty('itsm.telegram_bot.url');
        ss.info(`[TgBot] apiUrl: ${botUrl}/bot${this.getToken()}`);
