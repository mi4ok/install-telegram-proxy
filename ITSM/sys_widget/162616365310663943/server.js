(async () => {
    ...
    if (action === 'INIT') {
        data.botUrl = ss.getProperty('itsm.telegram_bot.url');
        data.slug = getSlug();
        data.instanseUri = getInstanseUri();
        data.webhookBaseUrl = ss.getProperty('itsm.telegram_bot.webhook_url') || data.instanseUri;
...
function getTranslations() {
    ...
    return {
        officialDocumentation: sm.getMessage('Telegram official documentation'),
        moreInformation: sm.getMessage('For more information about naming rules and other guidelines'),
        createNewBot: sm.getMessage('Create a new bot'),
        tokenExample: sm.getMessage('A token example'),
        method: sm.getMessage('method'),
        sendMessageToBotFather: sm.getMessage('Send the /newbot message to @BotFather'),
        giveNameToBot: sm.getMessage('Give a name and a username to the bot.'),
        changeOrCreateEndpoint: sm.getMessage('If needed, change or create endpoint'),
        anEndpoint: sm.getMessage('an endpoint'),
        linkToWebhookMethod: sm.getMessage('Link to webhook method'),
        token: sm.getMessage('Token'),
        endpoint: sm.getMessage('Endpoint'),
        copyAndPasteToken: sm.getMessage('Copy and paste token'),
        telegramBotConnection: sm.getMessage('Telegram bot connection'),
        chooseBotType: sm.getMessage('Choose the bot type'),
        connectedBots: sm.getMessage('Connected bots'),
        disconnectedBots: sm.getMessage('Disconnected bots'),
        youCanDisableConnection: sm.getMessage('You can disable the bot connection.'),
        deleteConnection: sm.getMessage('Delete the connection'),
        connectionIsDisabled: sm.getMessage('The connection is disabled'),
        connectionFailed: sm.getMessage('Connection failed'),
        messageConnected: sm.getMessage('Info message Connected'),
        buttonSet: sm.getMessage('button set'),
        step: sm.getMessage('Step'),
        info: sm.getMessage('info'),
        linkCreatedNewEndpoint: sm.getMessage('Link created new endpoint'),
        newEndpoint: sm.getMessage('New endpoint'),
        save: sm.getMessage('Save'),
        disable: sm.getMessage('Disable'),
        createNewEndpoint: sm.getMessage('Adding new record to table {tableTitle} was successful'),
    };
}
...
async function createConnection(token, endpoint, botId) {
    const record = new SimpleRecord('itsm_telegram_bot_connection');
    record.bot = botId;
    record.token = token;
...
function getTgBots() {
    const tgBots = {};
    const record = new SimpleRecord('itsm_telegram_bot');
